          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-green-50 border border-green-100 rounded-full text-[10px] font-bold text-green-700 tracking-wide">
                  {hasRoutine ? `${myRoutine.length} タスク` : 'はじめる'}
                </span>
                ...
                {(Object.keys(typeConfig) as (keyof typeof typeConfig)[]).map(type => {
                  ...
                  return (
                    <div key={type} className="flex items-center gap-3">
                      <span className={`text-[11px] font-bold tracking-wide w-10 ${cfg.text}`}>
                        {cfg.label}
                      </span>
        ...
        <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-stone-700">今後の予定</h3>
            </div>
          ...
          {upcomingEvents.length > 0 ? (
            ...
            <div className="text-center py-5">
              ...
              <p className="text-[10px] text-stone-300">「探す」からイベントを追加</p>
            </div>
          )}
        </div>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            {borrowHistory.length > 0 && (
              <span className="text-[10px] font-bold text-orange-500 bg-orange-50 px-2 py-0.5 rounded-full">
                計 {borrowHistory.reduce((s, r) => s + r.triedCount, 0)} 回
              </span>
            )}
          </div>
          {borrowHistory.length > 0 ? (
          ...
          <div className={`rounded-2xl border shadow-sm p-4 ${isPublished ? 'bg-green-50 border-green-100' : 'bg-white border-stone-100'}`}>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-stone-700">ルーティンを共有</h3>
              </div>
              {isPublished && (
            ...
            {isPublished ? (
              <div className="space-y-2">
                <p className="text-xs text-stone-500">あなたのルーティンが「探す」で公開されています。</p>
                <button
                  onClick={onUnpublish}
              ...
              <div className="space-y-2">
                ...
                {hasRoutine && (
                  <>
                    ...
                    <button
                      onClick={() => onPublish(publishTitle || `${displayName}の1日`)}
